...
- [`PATH_TRIMMING_HELPER_PLAN.md`](./PATH_TRIMMING_HELPER_PLAN.md) — implementation-facing v0 plan for bounded straight-segment trimming helpers (`trimSegment`, `trimSegmentStart`, `trimSegmentEnd`, `trimSegmentToCircle`) plus builder convenience (`trimmedLine`) with explicit no-curve/no-solver boundaries.
