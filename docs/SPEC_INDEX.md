...
- [`PATH_TRIMMING_HELPER_PLAN.md`](./PATH_TRIMMING_HELPER_PLAN.md) — implementation-facing v0 plan for bounded straight-segment trimming helpers (`trimSegment`, `trimSegmentStart`, `trimSegmentEnd`, `trimSegmentToCircle`) plus builder conveniences (`trimmedLine`, `circleToCircleLine`) with explicit no-curve/no-solver boundaries.
