...
| Placement | `relative-placement`, `mixed-nested-placement` | `ObjectPlacement`: `absolute`, `rightOf`, `leftOf`, `above`, `below` | Reference exists; parser fixture future | Assert relation kinds/fields lower correctly; compare resolved anchor relationships/gaps against reference semantics (not SVG text diffs). |
