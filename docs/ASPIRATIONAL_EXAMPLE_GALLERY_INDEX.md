...
| `aspirational-mechanism-lite` | `examples/aspirational_gallery/metapost/mechanism.mp` | `Level 1-2` | Static linkage / mechanism schematic | `primitives`, `intersections`, `annotation helpers`, `circular arcs / angle marks`, `style fields` | `manual coordinates`; `no source translation`; `no solver`; `no mechanics/physics simulation`; `no full visual fidelity`; `no clipping/gradients`; `no parser/JSON/AST support` | Add clearer joint labeling and richer linkage construction detail if more annotation conveniences become available. |
