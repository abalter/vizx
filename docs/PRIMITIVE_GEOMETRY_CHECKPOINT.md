...
- transforms beyond the current v0 translate+rotate+scale slice
...
- ordered transform operations now support `translate`, `rotate`, and `scale` in resolver-driven scene coordinates
- broader affine/local-frame features remain deferred
- rotate and scale support for `text` remains deferred in v0
