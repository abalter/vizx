...
- align (other than `alignLeft`, `alignX`, and `alignY`; edge alignment semantics are partially implemented)
- distribute
- Flowchart semantics
- Plotting or chart grammars
- Nonlinear constraints or a full constraint solver

The `alignment-reference` example remains a reference fixture for ongoing alignment and future distribution work. `alignLeft`, `alignX`, and `alignY` are implemented, while remaining edge alignments and distribution remain future work in [`docs/ALIGNMENT_DESIGN.md`](./ALIGNMENT_DESIGN.md).